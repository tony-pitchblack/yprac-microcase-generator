...
class ExpertStage:
    ...
    def _generate_microcase_description(self, comment: Dict, source_context: str) -> str:
        ...
        prompt_template = """        
        На основе комментария к коду и окружающего его контекста создай учебный микро-кейс, который раскрывает ту же корневую ошибку/принцип, но В ДРУГОЙ ТЕМЕ и с иным сюжетом, чем исходная задача студента.
Данные:
1. Файл студента: {file_path}
2. Строка: {line_number}
3. Комментарий: {comment}
4. Фрагмент исходного контекста (для понимания сути ошибки, НЕ для заимствования): {source_context}

Цели:
- Сфокусируйся на том же принципе/ошибке, указанной в комментарии.
- Прояви и объясни её в новом контексте и другой тематике.

Ограничения (обязательные):
- Не повторяй предметную область, историю, API, структуры данных, форматы, названия сущностей, переменные, функции, классы, пути и уникальные идентификаторы из кода студента.
- Не копируй входные/выходные данные, имена файлов, тестовые сценарии и специфику библиотек из исходного фрагмента.
- Используй новые нейтральные имена и иной сюжет предметной области. Совпадение допустимо только на концептуальном уровне ошибки.
- Не упоминай проект студента, его файлы или пути.

Требования к микро-кейсу:
1. Показывает ту же ошибку/принцип в новом контексте.
2. Даёт чёткую постановку задачи и ограниченный объём решения.
3. Может быть реализован в одном Python-файле.
4. Включает понятные требования к результату.
5. Не включай примеры кода или тесты — только описание задачи.

Оформление к микро-кейсу:
- В начале укажи название ошибки жирным.
- Далее — кратко опиши новый контекст, постановку задачи и критерии готовности.
